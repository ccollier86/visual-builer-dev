...
import template from './templates/tcm-monthly-standard.template.json';
import demoData from './sample-data/tcm-monthly-standard/demo-source.json';

async function main() {
	console.log('🚀 TCM Monthly Note (standard) test...');
	...
	try {
		...
		const base = `tcm-monthly-${timestamp}`;
		...
		if (cause) console.error('Causes:', cause);
		process.exit(1);
	}
}
