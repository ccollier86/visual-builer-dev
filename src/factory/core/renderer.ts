...
import { renderComponent, renderProvenance } from '../components/sections/content-sections';
import type { FactoryInputs, RenderOptions } from '../types';
...
export function renderNoteHTML(inputs: FactoryInputs): string {
	...
	injectHeadStyles(chunks, options?.styles);
	chunks.push(`</head>`);
	chunks.push(`<body class="note-body">`);
	chunks.push(`<div class="note-container">`);
	chunks.push(`<article class="note">`);

	const brandHeader = options?.brandOverrides?.headerHtml || tokens?.brand?.headerHtml || '';
	const brandLogo = options?.brandOverrides?.logoUrl || tokens?.brand?.logoUrl || '';

	if (brandHeader || brandLogo) {
		chunks.push(`<header class="note-brand-header">`);
		if (brandLogo) {
			chunks.push(`<img alt="Logo" class="note-brand-logo" src="${escapeAttr(brandLogo)}">`);
		}
		if (brandHeader) {
	...
	const brandFooter = options?.brandOverrides?.footerHtml || tokens?.brand?.footerHtml || '';
	if (brandFooter) {
...
function injectHeadStyles(headChunks: string[], styles?: RenderOptions['styles']): void {
	if (!styles) return;

	const { inlineScreen, inlinePrint, screenHref, printHref } = styles;

	if (screenHref) {
		headChunks.push(`<link rel="stylesheet" href="${escapeAttr(screenHref)}" media="screen">`);
	}

	if (printHref) {
		headChunks.push(`<link rel="stylesheet" href="${escapeAttr(printHref)}" media="print">`);
	}

	if (inlineScreen) {
		headChunks.push(`<style data-note-style="screen" media="screen">\n${inlineScreen}\n</style>`);
	}

	if (inlinePrint) {
		headChunks.push(`<style data-note-style="print" media="print">\n${inlinePrint}\n</style>`);
	}
}
